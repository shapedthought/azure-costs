...
class TestVeeamBackup(unittest.TestCase):
    ...
    def test_veeam_storage_costs(self):
        self.assertAlmostEqual(
            self.veeam_az_storage.calculate_storage_costs()[0].storage_per_month,
            14137.62,
            places=2,
        )
        self.assertAlmostEqual(
            self.veeam_az_storage.calculate_storage_costs()[0].snap_cost_month,
            108504.6,
            places=2,
        )
        self.assertAlmostEqual(
            self.veeam_az_storage.calculate_storage_costs()[0].api_per_month,
            14708.74,
            places=2,
        )
        self.assertAlmostEqual(
            self.veeam_az_storage.calculate_storage_costs()[0].total_storage_cost_month,
            137350.95,
            places=2,
        )
        self.assertAlmostEqual(
            self.veeam_az_storage.calculate_storage_costs()[0].total_storage_cost_year,
            1648211.419,
            places=2,
        )
        self.assertAlmostEqual(
            self.veeam_az_storage.calculate_storage_costs()[0].agent_cost_month,
            28846.356,
            places=2,
        )
        self.assertAlmostEqual(
            self.veeam_az_storage.calculate_storage_costs()[0].agent_cost_year,
            346156.272,
            places=2,
        )
